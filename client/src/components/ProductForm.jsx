  ...
  const handleFileChange = async (event) => {
    ...
    if (file) {
      ...
      try {
        ...
        console.log(VITE_REACT_APP_REMOVEBG_KEY)
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        ...
        if (res.data && res.data.brandOptions && res.data.categoryOptions) {
          setOptions(res.data);
        } else {
          console.error("Missing required options in response:", res.data);
        }
      } catch (error) {
